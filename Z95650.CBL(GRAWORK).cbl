      ...
      *----
       ...
       01  WS-WORK-AREA.
           ...
           05 WS-PROCESS-TYPE          PIC 9(01).
              88 WS-PROCESS-TYPE-VALID              VALUE 1 2 THRU 3 5.
           05 WS-SUB-AREA.
      ...
      *-----
       H110-FILE-CONTROL.
           IF (CHECK-FILE-INPUT NOT = 97) AND (CHECK-FILE-INPUT NOT = 0)
             DISPLAY "FAILED TO OPEN FILE.ERROR CODE:" CHECK-FILE-INPUT
              PERFORM H999-EXIT
           END-IF.
           IF (CHECK-FILE-OUTPUT NOT = 97) AND
      -       (CHECK-FILE-OUTPUT NOT = 0)
             DISPLAY "FAILED TO OPEN FILE.ERROR CODE:" CHECK-FILE-OUTPUT
              PERFORM H999-EXIT
           END-IF.
           IF (CHECK-FILE-INDEX NOT = 97) AND (CHECK-FILE-INDEX NOT = 0)
             DISPLAY "FAILED TO OPEN FILE.ERROR CODE:" CHECK-FILE-INDEX
              PERFORM H999-EXIT
           END-IF.
       H110-END. EXIT.

       H300-MOVE.
           COMPUTE WS-PROCESS-TYPE = FUNCTION NUMVAL(INP-PROCESS-TYPE)
           ...
           IF WS-PROCESS-TYPE-VALID
                 ...
                 END-EVALUATE
           ELSE
              DISPLAY 'INVALID TYPE' INP-PROCESS-TYPE
           END-IF.
      ...
      *---
       ...
       READ-RECORD.
           ...
           INVALID KEY MOVE 'RECORD NOT FOUND' TO OUT-DESC
           NOT INVALID KEY MOVE 'RECORD FOUND' TO OUT-DESC.
           MOVE WS-PROCESS-TYPE TO OUT-PRO-TYPE
       ...
       ADD-RECORD.
           COMPUTE IDX-FILE-ID=FUNCTION NUMVAL (INP-ID)
           COMPUTE IDX-FILE-DOVIZ=FUNCTION NUMVAL (INP-DOVIZ)
           MOVE 'CENGIZHAN' TO WS-STRING.
           MOVE 'DISCI' TO WS-WORK-STRING.
           MOVE WS-WORK-STRING TO IDX-FILE-NAME.
           MOVE WS-STRING TO IDX-FILE-SURNAME.
           MOVE '20230716' TO IDX-FILE-DATE.
       ...
       DELETE-RECORD.
           ...
           READ IDX-FILE KEY IS IDX-FILE-KEY
               INVALID KEY
                   MOVE 'RECORD NOT FOUND' TO OUT-DESC
               NOT INVALID KEY
                   DELETE IDX-FILE
                   MOVE 'RECORD DELETED' TO OUT-DESC.
           MOVE IDX-FILE-NAME TO OUT-FNAME-FROM
           MOVE IDX-FILE-SURNAME TO OUT-LNAME-FROM
